        ...
        <div className={`flex flex-col ${isCenter ? 'items-center text-center' : 'items-start text-left'}`}>
          ...
          <div className="mt-8 sm:mt-10 flex flex-col sm:flex-row items-center gap-3.5 sm:gap-4 w-full sm:w-auto">
            {primaryBtnText && (
              <Link
                href={primaryBtnHref}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 h-12 px-7 rounded-full bg-apple-dark hover:bg-black text-white font-medium text-sm sm:text-base transition-all duration-300 shadow-sm hover:shadow-md active:scale-95 group"
              >
                <span>{primaryBtnText}</span>
                <ArrowRight size={16} className="text-sembada-gold group-hover:translate-x-0.5 transition-transform" />
              </Link>
            )}
